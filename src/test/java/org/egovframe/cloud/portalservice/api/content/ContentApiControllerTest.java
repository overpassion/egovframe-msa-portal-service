...
class ContentApiControllerTest {
    ...
    void 컨텐츠_페이지_목록_조회() {
        ...
        assertThat(page.getContent())
                .isNotEmpty()
                .has(new Condition<>(l -> (CONTENT_NAME_PREFIX + "_10").equals(l.get(0).getContentName()), "ContentApiControllerTest.findPage contains " + CONTENT_NAME_PREFIX + "_10"))
                .has(new Condition<>(l -> (CONTENT_NAME_PREFIX + "_9").equals(l.get(1).getContentName()), "ContentApiControllerTest.findPage contains " + CONTENT_NAME_PREFIX + "_9"));
    ...
    void 컨텐츠_상세_조회() {
        ...
        // when
        ResponseEntity<ContentResponseDto> responseEntity = restTemplate.exchange(
                url,
                HttpMethod.GET,
                null,
    ...
    private void insertContents() {
        for (int i = 1; i <= GIVEN_DATA_COUNT; i++) {
            contents.add(contentRepository.save(Content.builder()
                    .contentName(CONTENT_NAME_PREFIX + "_" + i)
                    .contentRemark(CONTENT_REMARK_PREFIX + "_" + i)
                    .contentValue(CONTENT_VALUE_PREFIX + "_" + i)
                    .build()));
        }
    }
    ...
    private Content insertContent() {
        return contentRepository.save(Content.builder()
                .contentName(INSERT_CONTENT_NAME)
                .contentRemark(INSERT_CONTENT_REMARK)
                .contentValue(INSERT_CONTENT_VALUE)
                .build());
    }
